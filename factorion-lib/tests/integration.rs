...
use factorion_lib::{Consts, comment::*};
...
fn test_comment_new_very_big_number() {
    ...
    let comment = Comment::new(&very_big_number, (), Commands::NONE, MAX_LENGTH, "en")
        .extract(&consts)
        .calc(&consts);
    assert_eq!(comment.calculation_list, vec![]);
    assert_eq!(
...
fn test_float_overflow_approximate() {
    let consts = Consts::default();
    let comment = Comment::new("(44787927!)!", (), Commands::NONE, MAX_LENGTH, "en")
        .extract(&consts)
        .calc(&consts);
...
#[test]
fn test_same_in_different_writing() {
    let consts = Consts::default();
    let comment = Comment::new(
        "10000000000! (10^10)! (^(2)10)!",
        (),
        Commands::NONE,
        MAX_LENGTH,
        "en",
